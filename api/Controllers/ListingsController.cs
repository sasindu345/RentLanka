...
public class ListingsController : AuthorizedControllerBase
{
    ...
    public async Task<IActionResult> SeedListings([FromServices] AppDbContext context)
    {
        try
        {
            ...
            if (ownerId == null)
                return BadRequest(new { Error = "No users found. Please register a user first." });

            var listings = ListingSeeder.GetSeedListings(ownerId.Value);
            await context.Listings.AddRangeAsync(listings);
            await context.SaveChangesAsync();

            return Ok(new
            {
                Message = $"Successfully seeded {listings.Count} real-world listings.",
                SeededCount = listings.Count,
                Items = listings.Select(l => new { l.Title, l.Category, l.PricePerDay, l.District })
            });
        }
        catch (Exception ex)
        {
